...
3. Add the host to all relevant secret recipient lists (e.g. `agents/host/<hostname>/<NAME>.age` — see mbp0/m5 entries for the host-key-as-recipient pattern).
...
Use the `mbp0` `home.nix` or `_template-macos` as a starting template. The minimum viable skeleton is:
...
- [ ] **Add `home.nix`** (start from `mbp0` or `_template-macos`)
