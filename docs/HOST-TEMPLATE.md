...
> **Related**: See [AGENT-WORKFLOW.md](./AGENT-WORKFLOW.md) for how to keep config, docs, and tests in sync when making changes.
...
```text
hosts/<hostname>/
├── configuration.nix           # Main NixOS config (hokage + uzumaki)
├── hardware-configuration.nix  # Auto-generated (nixos-generate-config)
├── disk-config.zfs.nix         # ZFS layout (if using ZFS)
├── README.md                   # Host overview + Quick Reference
├── ip-<address>.md             # IP marker file (see below)
├── runbook-secrets.age         # Encrypted secrets for Git (NOT consumed by NixOS!)
│
├── docs/
...
└── tests/
    ...
    └── T00-nixos-base.sh       # Base system tests
```
...
| ---------------------------- | ---------------------------------------------- | --------------- |
| `configuration.nix`          | Main NixOS config with hokage/uzumaki settings | ✅ Yes          |
| `hardware-configuration.nix` | Auto-generated by `nixos-generate-config`      | ✅ Yes          |
| `disk-config.zfs.nix`        | Declarative ZFS disk layout (disko)            | ✅ Yes (if ZFS) |
...
| `ip-<address>.md` | IP marker for quick identification     | ✅ Yes   |

### Secrets & Tests

| File                         | Purpose                                          | Required |
| ---------------------------- | ------------------------------------------------ | -------- |
| `secrets/runbook-secrets.md` | Gitignored credentials reference (plain text)    | ✅ Yes   |
| `runbook-secrets.age`        | Encrypted backup for Git (NOT consumed by NixOS) | ✅ Yes   |
| `tests/README.md`            | Test documentation                               | ✅ Yes   |
| `tests/T00-nixos-base.sh`    | Base system verification                         | ✅ Yes   |

> ⚠️ **Important**: The `.age` file is purely for storing secrets in Git. NixOS does NOT consume it.
...
> The `.age` file is purely for backup — NixOS does NOT consume it.
...
> ⚠️ **SENSITIVE** - This file contains ALL credentials for <hostname>.
...
- [ ] Encrypt: `just encrypt-runbook-secrets <hostname>` → creates `runbook-secrets.age`
