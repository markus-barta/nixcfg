# pbek - Original NixOS Configuration (Historical)

> **Note**: This document contains the original content and setup instructions from when Patrizio Bekerle (pbek) created this NixOS configuration repository. The content has been preserved for historical reference but may contain outdated information. For current usage instructions, see [README.md](README.md).
...
This document preserves the original content from the pbek/nixcfg repository as it existed before modernization. Some commands and workflows may be outdated:

- Uses `make` commands instead of modern `just` commands
- References old agenix workflow with `~/.ssh/agenix`
