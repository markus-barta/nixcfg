...
let
  ...
  units = [
    ...
    # NOTE: nixfleet-agent (FleetCom) is deliberately NOT listed. It was decommissioned
    # and replaced by Pharos (pharos-beacon, a container, enumerated automatically).
    # Host truth surfaced its ghost — the board still carried a FleetCom card on five
    # hosts for a service that no longer exists anywhere — and the cards have now been
    # deleted. Reporting a unit that is *meant* to be absent would just be noise.
  ];
