...
echo 'Starting hsb1 repository cleanup.'
"$RESTIC_BIN" forget \
  --keep-last 12 \
...
if [ "$cleanup_status" -eq 0 ]; then
  ...
  result="Repository cleanup failed with exit status ${cleanup_status}. Review the hsb1 backup service."
fi
...
if [ -n "${MAIL_TO:-}" ]; then
  {
    printf 'To: %s\n' "$MAIL_TO"
    printf 'From: %s\n' "${MAIL_FROM:-$MAIL_TO}"
    printf 'Subject: Restic repository cleanup hsb1\n\n'
    printf '%s\n' "$result"
