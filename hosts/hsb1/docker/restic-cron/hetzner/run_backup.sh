...
echo 'Starting hsb1 backup.'
"$RESTIC_BIN" backup --host hsb1 \
  --exclude '*/cache/*' --exclude '*.log*' \
  /backup/home /backup/root /backup/etc \
  >"$message_file" 2>&1
restic_status="$?"
...
if [ "$restic_status" -eq 0 ]; then
  ...
  result="Backup failed with exit status ${restic_status}. Review the hsb1 backup service."
fi
