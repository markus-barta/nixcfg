...

{
  ...
  services.github-runners.csb1-start = lib.mkIf startRunnerSecretExists {
    enable = true;
    ...
    serviceOverrides = {
      ...
      # would block that. Keep docker.sock and the deploy directory reachable;
      # the registration token remains behind nixpkgs' default path boundary.
      ProtectHome = false;
      SupplementaryGroups = [
        "docker"
        "users"
