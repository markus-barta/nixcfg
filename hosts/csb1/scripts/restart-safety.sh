...
SERVER_ID="646294"
SERVER_NAME="v2202407214994279426"
...
echo "=== T16: Pre-Restart Safety Check ==="
echo "Server: $HOST (uptime 217+ days)"
echo
...
if [ -f "$TOKEN_FILE" ]; then
  ...
  echo "         Run T15 setup first for API restart capability"
  ((WARNINGS++))
fi
...
if [ "$READY" = true ] && [ $WARNINGS -eq 0 ]; then
  ...
  echo "6. Verify with: ssh -p 2222 mba@cs1.barta.cm 'uptime'"
  echo "7. Run: ./tests/T09-post-migration-verify.sh"
