...
declarative() {
  ...
  check_image_pin \
    ...
    '^ghcr\.io/inspr-at/janus/janus-engine:rust-engine-v0\.1\.14@sha256:[0-9a-f]{64}$'
  check_image_pin \
    pharos_release_pin \
    pharosd \
    '^ghcr\.io/inspr-at/pharos/pharosd:0\.1\.63@sha256:[0-9a-f]{64}$'
  ...
  check_command \
    ...
    '.schema_version == 1
     ...
     and .artifact.tag == "rust-engine-v0.1.14"
     and .artifact.digest == $digest
     and .artifact.development == false
     and .signature.verified == true
     and .signature.identity == "https://github.com/inspr-at/janus/.github/workflows/rust.yml@refs/tags/rust-engine-v0.1.14"
     ...
     and .provenance.source_ref == "refs/tags/rust-engine-v0.1.14"
