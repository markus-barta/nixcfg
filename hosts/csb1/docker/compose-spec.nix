...
# at runtime via agenix-decrypted bind. No plaintext secrets in this file.
{
  ...
  services = {
    ...
    # ============================================
    ppm = {
      image = "ghcr.io/inspr-at/paimos:5.6.1"; # explicit live pin; bump deliberately with the PAIMOS release/deploy flow. 5.6.1 = orchestrator lost-wakeup fix (PAI-725); 5.6.0 was per-language spec cache + header consolidation (PAI-734/735); 5.5.0 was voice cost gates + metering (PAI-724); 5.4.0 was ELI speak-back + configurable session budget (PAI-714); 5.3.3 was intake polish (PAI-721); 5.3.2 was voice lifecycle fixes (PAI-719); 5.3.1 was the mic Permissions-Policy fix (PAI-717); 5.3.0 was voice UX polish (PAI-715); 5.2.0 was ElevenLabs speech input (PAI-710); 5.1.0 was the Voice Intake epic (PAI-703). History: 4.8.0 sat here while live ran 5.0.0 (deploy flow sed-edits the yml, git restore reverted it) — a reconcile would have DOWNGRADED ppm over a 5.0.0-migrated DB. Caught in the OPS-116 QA, 2026-08-01.
      container_name = "ppm";
      restart = "unless-stopped";
      environment = [
        "PORT=8888"
    ...
    pharosd = {
      ...
      image = "ghcr.io/inspr-at/pharos/pharosd:0.1.72@sha256:164e9d39f092cacfe6ba300e2b1fa329a7797c5fd4d60593095a2f341d686fdf";
