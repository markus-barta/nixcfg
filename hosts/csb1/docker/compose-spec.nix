...
# at runtime via agenix-decrypted bind. No plaintext secrets in this file.
{
  ...
  services = {
    ...
    # ============================================
    ppm = {
      image = "ghcr.io/inspr-at/paimos:5.2.0"; # explicit live pin; bump deliberately with the PAIMOS release/deploy flow. 5.2.0 = ElevenLabs speech input (PAI-710); 5.1.0 was the Voice Intake epic (PAI-703). History: 4.8.0 sat here while live ran 5.0.0 (deploy flow sed-edits the yml, git restore reverted it) — a reconcile would have DOWNGRADED ppm over a 5.0.0-migrated DB. Caught in the OPS-116 QA, 2026-08-01.
      container_name = "ppm";
      restart = "unless-stopped";
      environment = [
        "PORT=8888"
