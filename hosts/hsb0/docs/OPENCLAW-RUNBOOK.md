...
- **2026-06-05**: Upgraded OpenClaw 2026.5.18 → 2026.6.1 on hsb0 (Merlin + Nimue). Driver: a 5.18 **telegram health-monitor flap** — the `getMe` health probe hit a ~1ms fetch-timeout → `[health-monitor] [telegram:merlin|nimue] restarting (reason: stopped)` every ~10–15 min (287 spurious restarts in ~2 days; channels actually worked between restarts, e.g. Nimue sent a message mid-flap). 6.1's health-monitor uses sane timing (`interval 300s` + startup/connect graces) → **0 restarts, 0 getMe timeouts** verified post-upgrade. **No config edits, no schema drift on boot** — `streaming` already object-form, TTS already `messages.tts.providers.elevenlabs` shape; pairings + memory survived. Done via `just oc-rebuild` (@latest → 6.1, ~5–10 min, no-cache). Same day: Ocean/dsc0 went 4.9 → 6.1 (needed the streaming string→object fix + an STT-gate `timeout` fix for 6.x's blocking `capability audio providers`). **Bridge unchanged**: hsb0 still runs the legacy log-tailing `0.2.0` agent-bridge (gateway-version-independent) — migration to the WS `0.6.x` model tracked in **FLEET-184**; dsc0's WS bridge bumped v3→v4 in **FLEET-183**. Percy/msbp left on 5.18 (checked: no active telegram flap, only latent getMe timeouts) — 6.1 upgrade ticketed in FLEET.
- **2026-05-19**: Upgraded OpenClaw 2026.5.7 → 2026.5.18 on both hsb0 (Merlin + Nimue) and miniserver-bp (Percy). Only two stable hops in the window (5.12 and 5.18); the rest were betas. **No schema migrations triggered** — live `openclaw.json` matches git source after rebuild on both hosts; the 5.18 doctor change that seeds `allowedOrigins` for `0.0.0.0`-style bind aliases did NOT fire even though both hosts use `bind: "lan"`. **Telegram pairings survived** on all three bots (4th upgrade in a row — pre-staged re-pair commands unused again). **Real downstream impact: FleetCom client (`vfleetcom/1.1.7`) on csb1 broke against hsb0** — gateway now requires protocol v4 (per 2026.5.12 release: "Gateway protocol: require v4 clients"), FleetCom client is on v3 → `[ws] protocol mismatch ... code=1002` errors every ~5s. Merlin/Nimue unaffected; only the FleetCom dashboard's OpenClaw chip will go stale until FleetCom is bumped. Percy not impacted (no FleetCom client polling msbp). **Percy bonus**: Mattermost (`@percy-ai`) confirmed connected at runtime (runbook listed it as Pending). One non-blocking warning surfaced: `channels.mattermost.interactions.callbackBaseUrl` defaults to loopback while `baseUrl` is `https://mattermost.bytepoets.com` — interactive button clicks won't round-trip until configured. Deferred. Entrypoint auto-push safety net fired on Percy boot (uncommitted workspace changes → `auto: pre-pull backup (container restart)`); benign.
- **2026-05-09**: Upgraded OpenClaw 2026.4.23 → 2026.5.7 (and Percy on miniserver-bp 2026.4.8 → 2026.5.7 in the same window). Three schema breaks hit, all fixed: (1) `messages.tts.elevenlabs` rejected by 4.25 TTS overhaul → stripped on first boot then re-added under new shape `messages.tts.providers.elevenlabs.{voiceId, modelId, ...}` with top-level `provider: "elevenlabs"`; ELEVENLABS_API_KEY env auto-picked up via the plugin's `providerAuthEnvVars` manifest entry; (2) `channels.telegram.accounts.<id>.streaming: "partial"` (string) rejected → migrated to object form `{ "mode": "partial" }` per the new `ChannelPreviewStreamingConfigSchema`; (3) Percy used legacy `streamMode: "partial"` rejected by strict() schema → same migration. **Bind-mount inode pitfall** found: `sed -i` on the host file breaks Docker file-bind mounts because sed renames; force-recreate (not just `docker restart`) is required after editing. **Secret leak hazard** documented: `openclaw doctor --fix` and `cat /home/node/.openclaw/openclaw.json` both print `${ENV_VAR}`-resolved secrets in clear (e.g. `BRAVE_API_KEY`) — agents must read/edit the git-source file only. Updated `+agents/rules/AGENTS.md`, this runbook's "View Live Config" section, and Percy's runbook in bpnixcfg with the warning. Telegram pairings survived the upgrade for all 3 bots (Merlin, Nimue, Percy). hsb0 NixOS gen 149 (was 147 / 2026-05-03; 96 commits had accumulated unsynced).
