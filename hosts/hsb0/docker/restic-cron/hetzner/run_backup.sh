...
echo 'Starting hsb0 backup.'
"$RESTIC_BIN" backup --host hsb0 \
  --exclude '*/cache/*' --exclude '*.log*' \
  /backup/var/lib/AdGuardHome /backup/var/lib/ncps /backup/etc \
  >"$message_file" 2>&1
restic_status="$?"
...
if [ "$restic_status" -eq 0 ]; then
  ...
  result="Backup failed with exit status ${restic_status}. Review the hsb0 backup service."
fi
