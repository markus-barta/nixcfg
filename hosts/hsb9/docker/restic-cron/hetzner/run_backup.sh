...
echo 'Starting hsb9 backup.'
"$RESTIC_BIN" backup --host hsb9 \
  --exclude '*/cache/*' --exclude '*.log*' \
  /backup/mounts/homeassistant /backup/mounts/mosquitto /backup/etc \
  >"$message_file" 2>&1
restic_status="$?"
...
if [ "$restic_status" -eq 0 ]; then
  ...
  result="Backup failed with exit status ${restic_status}. Review the hsb9 backup service."
fi
