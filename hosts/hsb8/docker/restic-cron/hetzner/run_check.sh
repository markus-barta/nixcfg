...
echo 'Starting hsb8 repository check.'
...
if [ "$check_status" -eq 0 ]; then
  ...
  result="Repository check failed with exit status ${check_status}. Review the hsb8 backup service."
fi
...
if [ -n "${MAIL_TO:-}" ]; then
  {
    printf 'To: %s\n' "$MAIL_TO"
    printf 'From: %s\n' "${MAIL_FROM:-$MAIL_TO}"
    printf 'Subject: Restic repository check hsb8\n\n'
    printf '%s\n' "$result"
