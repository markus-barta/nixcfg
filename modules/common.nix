...
in
{
  ...
  # evaluated without an exact source revision. This legacy single-value file
  ...
  home-manager.users = lib.genAttrs hokage.usersWithRoot (_userName: {
    ...
    programs = {
      # Starship: Managed by theme-hm.nix (no workaround needed with hokage.catppuccin.enable = false)
