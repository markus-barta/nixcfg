...
- 🔴 On a fleet host's `main`, the explicit fleet-safe refresh is `git pull --ff-only origin main` followed by `git submodule update --init doctrine`. On fleet hosts, `doctrine-private is intentionally absent`: private operator rules are not loaded, so do not perform private/operator workflows there. The shared `gitpl` alias pulls the current upstream and then uses this same submodule boundary; `gitsub` performs only the submodule step. Both update private doctrine only when that submodule was already initialized explicitly on an operator workstation. These aliases take no arguments; use the explicit Git commands when selecting a remote or branch. Never distribute private-repository credentials merely to make a bare recursive update succeed.
