...
- 🔴 Fleet-safe refresh is `git pull --ff-only origin main` followed by `git submodule update --init doctrine`. On fleet hosts, `doctrine-private is intentionally absent`: private operator rules are not loaded, so do not perform private/operator workflows there. The shared `gitpl`/`gitsub` aliases update private doctrine only when that submodule was already initialized explicitly on an operator workstation; never distribute private-repository credentials merely to make a bare recursive update succeed.
