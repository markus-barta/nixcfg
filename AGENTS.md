...
- 🔴 **Cross-repo authoring**: author changes only in the session's own repo; everywhere else file a ticket in the owning project with the proposed diff (reading is unrestricted, only writes are governed). One carve-out — release pins: edit only the pin + its comment, only via that repo's PR + checks (never a direct push to `main`, even where `main` is unprotected), with a recorded rollback path. Third-party / business-owned repos and no PR path → STOP and ask. Delete only branches you created.
