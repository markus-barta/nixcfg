...
# `.shared/common.just` is a gitignored symlink generated by devenv
# (devenv.yaml → imports shared/common → github:pbek/nix-shared). It only
# exists once a devenv/direnv shell has been entered in THIS checkout.
#
# `import?` (optional import) instead of `import`: a plain import is fatal, so
# in a fresh clone — or any account without direnv wired up — EVERY recipe died
# with "could not find source file for import", including `just bundle` and
# `just switch`. Shared recipes still load whenever devenv is active; without
# it, the host-local recipes below work on their own. Run `just shared` to see
# which mode you are in (NIX-216).
...
# NIX-216: report whether the devenv-provided shared recipes are loaded.
# `import?` above degrades silently by design, so this makes the two modes
# distinguishable instead of leaving you to guess why a shared recipe is
# "not found".
[group('config')]
shared:
    #!/usr/bin/env bash
    if [[ -e .shared/common.just ]]; then
        echo "✅ shared recipes LOADED (.shared/common.just present)"
        echo "   source: devenv → github:pbek/nix-shared"
    else
        echo "⚠️  shared recipes NOT loaded — .shared/common.just is missing."
        echo "   Host-local recipes (bundle, switch, safe-switch, …) work fine."
        echo "   To load the shared ones, enter the devenv shell in this checkout:"
        echo "       devenv shell          # one-off"
        echo "       direnv allow          # persistent, if direnv is set up"
    fi
